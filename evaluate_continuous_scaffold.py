"""Select a node-state MLM residual scale on validation, then test once."""
...
def main():
    ...
    parser.add_argument(
        ...
        default="artifacts/text_scaffold_topology_continuous",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=(-0.1, 0.0, 0.05, 0.1, 0.25, 0.5),
    )
    ...
    if not bool(config.get("continuous_semantic", False)):
        raise ValueError("the topology checkpoint has no continuous state")
    with open(
    ...
    def make_examples(split, window_seed, corruption_seed):
    ...
    validation_lengths, _, validation_unfinished, validation_states = (
        validation_rollout
    )
    selection = []
    for scale in args.scales:
        predictions = fill_sampled_scaffolds(
            ...
            sampled_states=validation_states,
            semantic_residual_scale=scale,
        )
        metrics = lexical_sampling_metrics(
            validation, predictions, validation_unfinished
        )
        selection.append({"scale": scale, **metrics})
        print(
            "scale={:.2f} matched_acc={:.4f} matched_edit={:.4f}".format(
                scale,
    ...
    test = make_examples(corpus["test"], data_seed + 403, data_seed + 101)
    ...
    lengths, rounds, unfinished, states = test_rollout
    predictions = fill_sampled_scaffolds(
        lexical_model,
        test,
        lengths,
        ...
        semantic_residual_scale=float(selected["scale"]),
    )
    ...
    output = os.path.join(
        args.topology_artifact_dir, "continuous_lexical_evaluation.json"
    )
