"""Paired comparison between two finite-regime depth-inside artifacts."""
...
def load_model(artifact_dir, vocab, device):
    ...
    model.load_state_dict(torch.load(
        os.path.join(artifact_dir, "shared_latent.pt"),
...
def nlls(model, examples, vocab, device, batch_size):
    values = []
    for start in range(0, len(examples), batch_size):
        joint, _, _, _, _ = shared_latent_log_likelihoods(
            model, examples[start:start + batch_size], vocab, device
        )
        values.append(-joint.cpu())
    return torch.cat(values)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--baseline-dir", default="artifacts/text_depth_inside_single_offset_frozen")
    parser.add_argument("--candidate-dir", default="artifacts/text_depth_inside_shared_latent_frozen")
    parser.add_argument("--output-dir", default="artifacts/text_depth_inside_shared_latent_variant_comparison")
    ...
    corpus = torch.load(os.path.join(config["data_dir"], "corpus.pt"), map_location="cpu", weights_only=True)
    ...
    baseline_nll = nlls(baseline, examples, vocab, device, args.batch_size)
    candidate_nll = nlls(candidate, examples, vocab, device, args.batch_size)
    comparison = paired_bootstrap(candidate_nll, baseline_nll)
    result = {
        "config": vars(args),
        "single_offset_joint_nll": float(baseline_nll.mean()),
        "two_regime_joint_nll": float(candidate_nll.mean()),
        "paired_two_regime_minus_single_offset": comparison,
    }
    os.makedirs(args.output_dir, exist_ok=True)
    with open(os.path.join(args.output_dir, "comparison.json"), "w", encoding="utf-8") as handle:
        json.dump(result, handle, indent=2)
    lines = [
        "# Two-regime versus single-offset control", "",
        "| Single offset NLL | Two regimes NLL | Difference | Paired SE | 95% CI |",
        "|---:|---:|---:|---:|---:|",
        "| {:.3f} | {:.3f} | {:+.3f} | {:.3f} | [{:+.3f},{:+.3f}] |".format(
            result["single_offset_joint_nll"], result["two_regime_joint_nll"],
